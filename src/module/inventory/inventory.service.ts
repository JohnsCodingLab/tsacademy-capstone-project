...
export class InventoryService {
  ...
  static async listProducts(
    ...
    const { search, categoryId, isArchived, lowStockOnly, sortBy, sortOrder } =
      query;
    ...
    if (lowStockOnly) {
      products = products.filter(
        (p) => p.reorderPoint !== null && p.stockLevel <= p.reorderPoint,
      );
  ...
  static async updateProduct(
    ...
    logger.info(
      { productId, orgId, actorId, changes: data, event: "PRODUCT_UPDATED" },
      "Product updated",
    );
  ...
  static async restoreProduct(
    ...
    if (!product) throw new AppError("Product not found", 404);
    if (!product.isArchived) throw new AppError("Product is not archived", 409);
  ...
  static async adjustStock(
    ...
    const movement = await prisma.$transaction(async (tx) => {
      ...
      if (!product) throw new AppError("Product not found", 404);
      if (product.isArchived) {
        throw new AppError("Cannot adjust stock for an archived product", 409);
      }
   ...
   */
  static async getLowStockAlerts(orgId: string): Promise<ProductResponseDTO[]> {
    ...
    return products
      .filter((p) => p.reorderPoint !== null && p.stockLevel <= p.reorderPoint)
      .map(sanitizeProduct);
  }
  ...
  static async getForecast(
    ...
    const forecasts: ForecastDTO[] = products.map((p) => {
      const totalOut = p.stockMovements.reduce((sum, m) => sum + m.quantity, 0);
      const avgDailySalesVelocity =
        totalOut > 0 ? parseFloat((totalOut / windowDays).toFixed(4)) : null;
      ...
      const isLowStock =
        p.reorderPoint !== null ? p.stockLevel <= p.reorderPoint : false;
